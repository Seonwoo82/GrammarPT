...
export const sendTelemetry = async (payload, options = {}) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const warnPayload = { status: response.status, body: errorText, eventType, sessionId, userId };
      console.warn("telemetry send 실패: bad response", warnPayload);
      if (debug) {
        console.info("[telemetry][debug]", { ok: false, ...warnPayload });
      }
      return { ok: false, status: response.status, sessionId, userId, eventType };
    }
    if (debug) {
      console.info("[telemetry][debug]", { ok: true, status: response.status, sessionId, userId, eventType });
    }
    return { ok: true, status: response.status, sessionId, userId, eventType };
  } catch (error) {
    // 베스트 에포트: 사용자 동선을 막지 않음
    console.warn("telemetry send 실패:", error);
    if (debug) {
      console.info("[telemetry][debug]", { ok: false, error: error?.message, eventType: payload?.eventType });
    }
    return { ok: false, status: null, sessionId: payload?.sessionId, userId: payload?.userId, eventType: payload?.eventType };
  }
};
